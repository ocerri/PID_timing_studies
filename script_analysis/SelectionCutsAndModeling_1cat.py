...
binning_vtx = [100, 0, 2000, 100, 0, 800]
...
for path in file_path:
    ...
    binning_mass = [80,stopmass/2.5,stopmass*2.5]
    arr_mh = t['mh'][kin_sel]
    h_mass = create_TH1D(arr_mh, 'h_mass_'+name, name, binning=binning_mass, axis_title=['m_{H} [GeV]','Events / {:.1} GeV'.format(float(binning_mass[2]-binning_mass[1])/binning_mass[0])])
    ...
    f_fit = rt.TF1("f_fit",'[0]*(' + left_fun + " + " + central_fun + ' + ' + right_fun + ')', np.percentile(arr_mh, 2) , np.percentile(arr_mh, 98))
