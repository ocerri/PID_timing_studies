...
using namespace RooFit;
 ...
 double RooGausDoubleExp::evaluate() const
 {
   ...
   if(t>-alpha1 && t<alpha2){
     ...
     double A1 = exp(0.5*alpha1*alpha1);
     return A1*exp(alpha1*t);
   }else if(t>=alpha2){
     double A2 = exp(0.5*alpha2*alpha2);
     return A2*exp(-alpha2*t);
   }//else{
 ...
 Double_t RooGausDoubleExp::analyticalIntegral(Int_t code, const char* rangeName) const
 {
   ...
   if(left_low < left_high){ //is the left tail in range?
     left = A1*width/(alpha1)*(exp((left_high-mean)*invwidth*alpha1) - exp((left_low-mean)*invwidth*alpha1));
   }
   ...
   if(right_low < right_high){ //is the right tail in range?
     right = -A2*width/(alpha2)*(exp(-(right_high-mean)*invwidth*alpha2) - exp(-(right_low-mean)*invwidth*alpha2));
   }
