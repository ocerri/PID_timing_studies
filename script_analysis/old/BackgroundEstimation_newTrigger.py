...
peak_instant_lumi = 7e-5 # fb-1s-1
donotdelete = []
...
min_vtx_pt = 100
mass_lower_bound = 50
binning_vtx = [100, 0, 3000, 50, 0.01, 5]
binning_pt = [100, 0, 600, 50, 0.01, 5]
binning_mass = [1 + (300-mass_lower_bound)/10, mass_lower_bound, 300.]
...
c_summary.Divide(3,len(file_path))
...
for i, path in enumerate(file_path):
    ...
    kin_sel = np.logical_and(kin_sel, (t['mh']+ t['ml'])/2. > mass_lower_bound)

    arr_mh = (t['mh'][kin_sel] + t['ml'][kin_sel])/2.
    h_mass[label] = create_TH1D(arr_mh, 'h_mass_'+name, name, binning=binning_mass, axis_title=['(m_{{H}} + m_{{L}})/2 [GeV]','Events / {:.1} GeV'.format(float(binning_mass[2]-binning_mass[1])/binning_mass[0])])
...
print "Peak rate @ {:1.2e} fb-1s-1 = {:1.2e} Hz".format(peak_instant_lumi, eff_xsec * peak_instant_lumi)
