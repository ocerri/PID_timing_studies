...
void Set_GausDoubleExpPdf_pars(RooWorkspace* w, double mass, TString label="", bool is_fix = true) {
  w->var("mu"+label)->setVal(mass*9.99e-01 + 3.28e-03);
  w->var("sigma"+label)->setVal(-2.74e-08*pow(mass,3) + 7.52e-05*mass*mass + -4.97e-03*mass + 2.84e+00);
  w->var("aL"+label)->setVal(3.01e-04*mass + 8.06e-01);
  w->var("aR"+label)->setVal(2.42e-04*mass + 8.50e-01);
...
double GetEfficiency(double mass) {
  vector<double> val_m = {50, 100, 200, 350, 500, 750, 1000};
  double val_eff[] = {1.07e-03, 1.67e-02, 7.45e-02, 1.39e-01, 1.67e-01, 1.87e-01, 1.92e-01};
...
RooWorkspace* ModelTOFAnalysis_RHad(int nsig = 0, int nbkg = 0 )
{
  ...
  // Create the fitting model
  w->factory(Form("Exponential:bkg_pdf(Mass[%f,%f], lambda[-0.0413,-5,-0.001])",binning[1], binning[2]));
  // w->factory(Form("PowerLaw:bkg_pdf(Mass[%f,%f], power[-3.95, -10, -1])",binning[1], binning[2]));
  w->factory("GausDoubleExp:sig_pdf(Mass, mu[200., 100, 4000], sigma[30., 10, 500], aL[2, 0.1, 3], aR[1.5, 0.1, 3])");
  w->factory("SUM:model(nsig[0,10000]*sig_pdf, nbkg[3000,0,1000000000]*bkg_pdf)");  // for extended model
...
void TOFAnalysis_RHad_2cat(){
  ...
  for (double mass = 100; mass <= 1000; mass += 100) {
    Set_GausDoubleExpPdf_pars(w, mass);

    HypoTestInvTool calc(&optHTInv);
    int nsig_max = 30;
    if( mass < 200) nsig_max = 250;
