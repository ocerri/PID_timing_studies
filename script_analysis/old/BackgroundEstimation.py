...
sel_line = rt.TLine()
sel_line.SetLineColor(2)
sel_line.SetLineWidth(2)
sel_line.SetLineStyle(7)
...
h_pt = {}
binning_pt = [100, 0, 3000, 100, 0, 1500]


h_mass = {}
mass_lower_bound = 30
binning_mass = [100, mass_lower_bound, 1000.]

c_pt = rt.TCanvas('c_ptbins', 'c_ptbins', 800, 1600)
c_pt.Divide(2,len(file_path))
...
for i, path in enumerate(file_path):
    ...
    h_pt[label] = create_TH2D(arr_pt_vtxpt, 'h_pt_'+name, 'SM particle generated tracks, HardQCD #hat{p}_{T} '+label+' GeV', binning=binning_pt, axis_title=['reco vtx sum p_{T} [GeV]', 'reco p_{T} [GeV]'])

    sel = np.logical_and(tree[label]['vtx_SumPT'] > min_vtx_pt, tree[label]['pt'] > min_trk_pt)
    sel = np.logical_and(sel, tree[label]['M_reco']>mass_lower_bound)
    arr_mass = tree[label]['M_reco'][sel]
    h_mass[label] =  create_TH1D(arr_mass, 'h_mass_'+name, 'Mass spectrum of HardQCD #hat{p}_{T} '+label+' GeV', binning=binning_mass, axis_title=['Mass reco [GeV]', 'Events / {} GeV'.format(binning_mass[2]/binning_mass[0])])
    ...
    if h_mass[label].GetEntries() > 10 :
        r = h_mass[label].Fit('expo', 'ILQRS', '', mass_lower_bound, binning_mass[2])
        h_mass[label].fit_pars = [r.Parameter(0), r.Parameter(1)]
        rt.gPad.SetLogy()
        print "Mu = {:.1f} \pm {:.1f}".format( 1./r.Parameter(1), r.ParError(1)/r.Parameter(1)**2)


    # Getting the xsec
    production[label] = GetXsec(path)
    print 'Passed', h_mass[label].GetEntries()

    #Computing the weights
    h_mass[label].weight = h_mass[label].GetEntries() * production[label][0] / production[label][2]
    weight_sum += h_mass[label].weight
    print 'Weight: ', h_mass[label].weight

c_pt.Update()

# Draw a weughted sum of the bkg
c_sum = rt.TCanvas("c_sum", "c_sum", 800, 600)
c_sum.donotdelete = []
h_stack = create_TH1D([], 'h_mass_sum', 'Mass spectrum of HardQCD weighted', binning=binning_mass, axis_title=['Mass reco [GeV]', 'Norm. entries / {} GeV'.format(binning_mass[2]/binning_mass[0])])
h_stack.Sumw2()
...
f_fit_sum = rt.TF1('f_fit_sum', '[0] * exp (-[1]*x)/ x ', binning_mass[1]+5, binning_mass[2])
res = h_stack.Fit('f_fit_sum','SL0RI+')
...
c_sum.Update()
print 'Normalized bkg fit parameters', f_fit_sum.GetParameter(0)/f_fit_sum.Integral(binning_mass[1]+5, 1e5), f_fit_sum.GetParameter(1)
...
c_prod.SetLogx()
c_prod.Update()
# c_prod.SaveAs('/Users/olmo/Desktop/trial.root')
