...
TH1 *h_sum_ILD = new TH1F("h_sum_ILD","Missing Mass in Z#rightarrow #mu^{+}#mu^{-} tagged events (M_{Z}+/- 4 GeV)",60,40.0,160.0 );
TH1 *h_sum_CMS = new TH1F("h_sum_CMS","Missing Mass in Z#rightarrow #mu^{+}#mu^{-} tagged events (M_{Z}+/- 4 GeV)",60,40.0,160.0 );
...
TString paper2012_cuts = "dtheta_ll_lab>100&&Pt_ll>10&&Pl_ll<50&&acoplanarity_angle>10&&M_ll>87&&M_ll<95&&is_mumu==1"; // modified cuts
...
void plot_M_miss_tails_mumu(){
  ...
  TLegendEntry *entry=leg->AddEntry("NULL","#intL=3.5ab^{-1}, #sqrt{s}=240GeV, BR_{H->inv}=100%","h");

  //TLegendEntry *entry=leg->AddEntry("NULL","Unit normalized, #sqrt{s}=240GeV, BR_{H->inv}=100%","h");
