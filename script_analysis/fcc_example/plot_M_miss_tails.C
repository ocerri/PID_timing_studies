...
Int_t binM = 10; //Multiplies the number of bins
...
TH1 *h_sum_ILD = new TH1F("h_sum_ILD","Missing Mass in Z#rightarrow l^{+}l^{-} tagged events (M_{Z}+/- 4 GeV)",60*binM,40.0,160.0 );
TH1 *h_sum_CMS = new TH1F("h_sum_CMS","Missing Mass in Z#rightarrow l^{+}l^{-} tagged events (M_{Z}+/- 4 GeV)",60*binM,40.0,160.0 );
...
//TString paper2012_cuts = "dtheta_ll_lab>100&&Pt_ll>10&&Pl_ll<50&&acoplanarity_angle>10&&M_ll>87&&M_ll<95&&is_ee==1"; // modified cuts for mumu only
...
void plot_M_miss_tails(){
  ...
  //h_sum_ILD->Scale(1/h_sum_ILD->Integral());
  //h_sum_CMS->Scale(1/h_sum_CMS->Integral());
  ...
  h_sum_ILD->Sumw2();
  h_sum_ILD->SetYTitle(Form("Events / %.4f GeV",2/(float)binM));
  ...
  h_sum_CMS->Sumw2();
  h_sum_CMS->SetYTitle(Form("Events / %.4f GeV",2/(float)binM));
  ...
  leg->SetFillColor(0);
  leg->SetFillStyle(0);
  TLegendEntry *entry=leg->AddEntry("NULL","L=3.5ab^{-1}, #sqrt{s}=240GeV, BR_{H->inv}=100%","h");
