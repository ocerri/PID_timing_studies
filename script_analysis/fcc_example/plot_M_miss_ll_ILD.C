...
//Merge code -------------------------------------------------------------------
void plot_M_miss_ll_ILD(TString cuts = "paper2012", const char* outtag="cuts"){
  ...
  leg1->AddEntry(h_HZ_miss,Form("Signal"),"l");
  leg1->AddEntry(h_ZZ_miss,Form("ZZ"),"l");
  leg1->AddEntry(h_WW_miss,Form("WW"),"l");
  leg1->AddEntry(h_ZZ_miss,Form("All backgrounds"),"f");
  leg1->Draw();

  leg = new TLegend(0.1380417,0.787844,0.6845907,0.891055,NULL,"brNDC");
  leg->SetBorderSize(1);
  ...
  leg->SetFillColor(0);
  leg->SetFillStyle(0);
  entry=leg->AddEntry("NULL","ILD at FCC-ee, 3.5ab^{-1}, #sqrt{s}=240GeV, BR_{H->inv}=100%","h");
