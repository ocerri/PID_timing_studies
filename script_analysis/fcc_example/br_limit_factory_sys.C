...
void br_limit_factory_sys(Int_t binM = 1, unsigned int trials = 100, Int_t seed_st = 0){
  Double_t displ = (160.0-40.0)/(2*(float)binM*60.0);

  std::ofstream logfile;
  logfile.open(Form("log/br_limit_factory_sys_%d_%d_%d.log",binM,seed_st,seed_st+trials-1));
  ...
  TFile *f_out = TFile::Open(Form("_root/br_limit_factory_sys_binM%d_%d_%d_%.0f.root",binM,seed_st,seed_st+trials-1, Luminosity),"RECREATE");
  f_out->cd();
  h_br->Write();
