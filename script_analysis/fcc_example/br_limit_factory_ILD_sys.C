...
//Int_t binM = 1; //Multiply the number of bin

void br_limit_factory_ILD_sys(Int_t binM = 1, unsigned int trials = 10, Int_t seed_st = 0){
  Double_t displ = (160.0-40.0)/(2*(float)binM*60.0);

  std::ofstream logfile;
  logfile.open(Form("log/br_limit_factory_ILD_sys_%d_%d_%d.log",binM,seed_st,seed_st+trials-1));
  ...
  //Fill the histos-------------------------------------------------------------
  TH1* h_WW_miss = new TH1D("h_WW_miss","h_WW_miss",60*binM,40.0+displ,160.0+displ);
  TH1* h_ZZ_miss = new TH1D("h_ZZ_miss","h_ZZ_miss",60*binM,40.0+displ,160.0+displ);
  TH1* h_HZ_miss = new TH1D("h_HZ_miss","h_HZ_miss",60*binM,40.0+displ,160.0+displ);
  ...
  h_sg->Sumw2();
  TH1 *h_bg = new TH1F("h_bg","h_bg",60*binM,40.0+displ,160.0+displ);
  ...
  TH1 *h_br = new TH1F("h_br","BR limit distribution, ILD card", 50, 0.0,5.0);
  ...
  TFile *f_out = TFile::Open(Form("_root/br_limit_factory_ILD_sys_%d_%d_%d_%.0f.root",binM, seed_st,seed_st+trials-1, Luminosity),"RECREATE");
  f_out->cd();
  h_br->Write();
