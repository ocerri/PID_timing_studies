...
void br_limit_factory_ILD(Int_t binM = 1, unsigned int trials = 100, Int_t seed_st = 0){
  std::ofstream logfile;
  logfile.open(Form("log/br_limit_factory_ILD_%d_%d_%d.log",binM,seed_st,seed_st+trials-1));
  ...
  TH1 *h_br = new TH1F("h_br",Form("BR limit distribution, ILD card, binning %.1f MeV", 2000.0/binM), 50, 0.0,5.0);
  h_br->SetXTitle("BR [per cent]");
  h_br->SetYTitle("Number of toymodels");

  //Histo filling loop----------------------------------------------------------
  for (size_t i = 0; i < trials; i++) {
    logfile << "Iteration " << i+1 << "/" << trials << std::flush;

    Double_t br_tmp = find_br_upper_limit(seed_st+i, h_bg, h_sg, h_WW_miss, h_ZZ_miss, h_HZ_miss);
    h_br->Fill(br_tmp);

    logfile << "   br: " << br_tmp  << std::endl << std::flush;

  }

  TFile *f_out = TFile::Open(Form("_root/br_limit_factory_ILD_%d_%d_%d_%.0f.root",binM,seed_st,seed_st+trials-1, Luminosity),"RECREATE");
  f_out->cd();
  h_br->Write();
