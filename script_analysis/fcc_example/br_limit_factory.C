...
void br_limit_factory(Int_t binM = 1, unsigned int trials = 1, Int_t seed_st = 0){
  std::ofstream logfile;
  logfile.open(Form("log/br_limit_factory_%d_%d_%d.log",binM,seed_st,seed_st+trials-1));
  ...
  TFile *f_out = TFile::Open(Form("_root/br_limit_factory_%d_%d_%d_%.0f.root",binM,seed_st,seed_st+trials-1, Luminosity),"RECREATE");
  f_out->cd();
  h_br->Write();
